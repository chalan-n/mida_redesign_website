...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $phone && $car_type) {
        try {
            $sql = "INSERT INTO loan_applications (loan_type, name, phone, line_id, car_type, car_brand, car_model_year, debt_status, loan_amount, created_at) VALUES ('title_loan', ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            $stmt = $db->prepare($sql);
            $stmt->execute([$name, $phone, $line_id, $car_type, $car_brand, $car_model_year, $debt_status, $loan_amount]);
...
<head>
    ...
    <title>สมัครสินเชื่อจำนำทะเบียน - MIDA LEASING</title>
    <meta name="description" content="สมัครสินเชื่อจำนำทะเบียนรถยนต์ออนไลน์ อนุมัติไว ไม่โอนเล่ม รับเงินก้อนใหญ่">
...
<body>
    ...
    <div class="register-header">
        <div class="container">
            <h1 style="margin-bottom: 10px; color: var(--accent-gold);">สมัครสินเชื่อจำนำทะเบียน</h1>
