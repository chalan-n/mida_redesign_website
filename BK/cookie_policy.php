...
<head>
    ...
    <title>นโยบายเกี่ยวกับคุกกี้ (Cookies Policy) - MIDA LEASING</title>
    <meta name="description" content="นโยบายเกี่ยวกับคุกกี้ บริษัท ไมด้าลิสซิ่ง จำกัด (มหาชน)">
...
<body>
    ...
    <section class="content-section">
        ...
        try {
            ...
            // Track visitor
            @include_once 'track_visitor.php';
            $stmt = $db->prepare("SELECT title, content FROM pages WHERE slug = 'cookie_policy'");
            $stmt->execute();
            $page = $stmt->fetch();
        ...
        <div class="container policy-content">
            <?php if ($content): ?>
                <?php echo $content; ?>
            <?php else: ?>
                <h2>นโยบายเกี่ยวกับคุกกี้ (Cookie Policy)</h2>
                <p>ขออภัย ไม่พบข้อมูลนโยบายเกี่ยวกับคุกกี้ในขณะนี้</p>
            <?php endif; ?>
        </div>
    </section>
