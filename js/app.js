...
const BluebookApp = {
    ...
    updateBreadcrumb() {
        ...
        const safeCid = this.escapeJsString(this.state.cid);
        const safeBid = this.escapeJsString(this.state.bid);
        const safeMid = this.escapeJsString(this.state.mid);
        let html = `
        ...
        if (this.state.cid) {
            ...
            html += `<a href="#" class="breadcrumb-item ${this.state.currentView === 'brands' ? 'active' : ''}" onclick="BluebookApp.loadBrands('${safeCid}'); return false;">${typeName}</a>`;
        }

        if (this.state.bid) {
            html += `<span class="breadcrumb-separator"><i class="fas fa-chevron-right"></i></span>`;
            html += `<a href="#" class="breadcrumb-item ${this.state.currentView === 'models' ? 'active' : ''}" onclick="BluebookApp.loadModels('${safeCid}', '${safeBid}'); return false;">${this.escapeHtml(this.state.bid)}</a>`;
        }

        if (this.state.mid) {
            html += `<span class="breadcrumb-separator"><i class="fas fa-chevron-right"></i></span>`;
            html += `<a href="#" class="breadcrumb-item ${this.state.currentView === 'years' ? 'active' : ''}" onclick="BluebookApp.loadYears('${safeCid}', '${safeBid}', '${safeMid}'); return false;">${this.escapeHtml(this.state.mid)}</a>`;
        }
    ...
    renderHome(types) {
        ...
        types.forEach(type => {
            html += `
                <div class="card card-type" onclick="BluebookApp.loadBrands('${this.escapeJsString(type.id)}')">
                    <div class="card-icon">
                        <i class="fas ${this.escapeHtml(type.icon)}"></i>
                    </div>
                    <h3 class="card-title">${this.escapeHtml(type.name)}</h3>
                </div>
            `;
        });
    ...
    renderBrands(brands) {
        ...
        brands.forEach(brand => {
            html += `
                <div class="card" onclick="BluebookApp.loadModels('${this.escapeJsString(this.state.cid)}', '${this.escapeJsString(brand.name)}')">
                    <div class="card-icon">
                        <img src="icons/${this.escapeHtml(brand.icon)}" alt="${this.escapeHtml(brand.name)}" onerror="this.parentElement.innerHTML='<i class=\\'fas fa-car\\'></i>'">
                    </div>
                    <h3 class="card-title">${this.escapeHtml(brand.name)}</h3>
                </div>
            `;
        });
    ...
    renderModels(models) {
        ...
        let html = `
            <button class="back-btn" onclick="BluebookApp.loadBrands('${this.escapeJsString(this.state.cid)}')">
                <i class="fas fa-arrow-left"></i> กลับ
            </button>
            <div class="section-title mt-4">
                <h2><i class="fas fa-car-side"></i> ${this.escapeHtml(this.state.bid)}</h2>
        ...
        if (models.length === 0) {
            ...
            models.forEach(model => {
                html += `
                    <div class="card" onclick="BluebookApp.loadYears('${this.escapeJsString(this.state.cid)}', '${this.escapeJsString(this.state.bid)}', '${this.escapeJsString(model.name)}')">
                        <div class="card-icon">
                            <i class="fas ${this.state.cid == '1' ? 'fa-truck-pickup' : 'fa-car-side'}"></i>
                        </div>
                        <h3 class="card-title">${this.escapeHtml(model.name)}</h3>
                    </div>
                `;
            });
        }
    ...
    renderYears(years) {
        ...
        let html = `
            <button class="back-btn" onclick="BluebookApp.loadModels('${this.escapeJsString(this.state.cid)}', '${this.escapeJsString(this.state.bid)}')">
                <i class="fas fa-arrow-left"></i> กลับ
            </button>
            <div class="section-title mt-4">
                <h2><i class="fas fa-calendar-alt"></i> ${this.escapeHtml(this.state.bid)} ${this.escapeHtml(this.state.mid)}</h2>
                <p>เลือกปี</p>
            </div>
            <div class="cards-grid">
        `;
        ...
        if (years.length === 0) {
            ...
            years.forEach(year => {
                html += `
                    <div class="card" onclick="BluebookApp.loadSubModels('${this.escapeJsString(this.state.cid)}', '${this.escapeJsString(this.state.bid)}', '${this.escapeJsString(this.state.mid)}', '${this.escapeJsString(year.year)}')">
                        <div class="card-icon">
                            <i class="fas fa-calendar"></i>
                        </div>
                        <h3 class="card-title">${this.escapeHtml(year.year)}</h3>
                    </div>
                `;
            });
        }
    ...
    renderSubModels(submodels) {
        ...
        let html = `
            <button class="back-btn" onclick="BluebookApp.loadYears('${this.escapeJsString(this.state.cid)}', '${this.escapeJsString(this.state.bid)}', '${this.escapeJsString(this.state.mid)}')">
                <i class="fas fa-arrow-left"></i> กลับ
            </button>
            <div class="section-title mt-4">
        ...
        if (submodels.length === 0) {
            ...
            submodels.forEach(sub => {
                html += `
                    <div class="card" onclick="BluebookApp.loadPrice('${this.escapeJsString(this.state.cid)}', '${this.escapeJsString(this.state.bid)}', '${this.escapeJsString(this.state.mid)}', '${this.escapeJsString(this.state.yy)}', '${this.escapeJsString(sub.id)}')">
                        <div class="card-icon">
                            <i class="fas fa-car"></i>
                        </div>
                        <h3 class="card-title">${this.escapeHtml(sub.submodel)}</h3>
    ...
    renderPrice(car) {
        ...
        let html = `
            <button class="back-btn" onclick="BluebookApp.loadSubModels('${this.escapeJsString(this.state.cid)}', '${this.escapeJsString(this.state.bid)}', '${this.escapeJsString(this.state.mid)}', '${this.escapeJsString(this.state.yy)}')">
                <i class="fas fa-arrow-left"></i> กลับ
            </button>
            ...
            <div class="price-page">
                ...
                <div class="price-header">
                    ...
                    <h1 class="price-header-title">${this.escapeHtml(this.state.bid)}</h1>
                    <p class="price-header-subtitle">${this.state.mid} | ปี ${this.state.yy}</p>
                </div>
                
                ${car.picture ? `
                    <div class="price-image-container">
                        <img src="${this.escapeHtml(car.picture)}" alt="Car Image" class="price-image" onerror="this.parentElement.style.display='none'">
                    </div>
                ` : ''}
                ...
                <div class="price-details">
                    <div class="price-detail-item">
                        ...
                        <div class="price-detail-content">
                            ...
                            <span class="price-detail-value">${this.escapeHtml(car.code)}</span>
                        </div>
                    </div>
                    <div class="price-detail-item">
                        ...
                        <div class="price-detail-content">
                            ...
                            <span class="price-detail-value">${this.escapeHtml(car.submodel)}</span>
                        </div>
                    </div>
                    <div class="price-detail-item">
                        ...
                        <div class="price-detail-content">
                            ...
                            <span class="price-detail-value">${this.escapeHtml(car.gear)}</span>
                        </div>
                    </div>
                </div>
            </div>
